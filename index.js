...
const assertOrDie = (condition, message, program) => {
  if (!condition) {
...
const signalOnFileChange = (target, dirToWatch) => {
  assert(
    statSync(dirToWatch).isDirectory(),
    `Directory to watch ${dirToWatch} was not found or is not a directory`
  );

  const watcher = new sane(dirToWatch, { glob: '*' });

  const notifyTarget = async (file, root, stat) => {
    console.log(`Event: changed, File: ${file}; signalling restart.`);
    try {
  ...
  watcher.on('change', notifyTarget);
