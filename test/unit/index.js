  ...
  describe('Signal on File Change', () => {
    ...
    describe('File Watcher', () => {
      ...
      afterEach(() => {
        ...
        fs.unlinkSync(fileThatWillChange);
      });
      ...
      it('should watch the file and send the SIGHUP signal on change', async () => {
        ...
        watcher = signalOnFileChange(target, __dirname);
        fs.appendFileSync(fileThatWillChange, 'foobar\n');
        await Promise.delay(500);
        return expect(target.kill).to.be.calledWith({ signal: 'SIGHUP' });
      });
    });
  });
});
